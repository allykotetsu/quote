...
pub(crate) fn function(command_context: &CommandContext) -> Result<(), Error> {
    // Get page or 1.
    let Ok(page) = command_context.args.get(0).unwrap_or(&"1".to_string()).parse::<u32>() else {
        // If there was an error parsing page, then tell user how to use command.
        return Ok(command_context.reply("`!quotes` requires one whole, positive number when using the command this way.\n\n**Example**: `!quotes 1`"))
    };
